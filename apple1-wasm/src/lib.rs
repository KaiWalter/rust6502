...
pub fn start() {
    ...
    let mut compact_apple1 = Apple1Compact::new(tx_apple_output, rx_apple_output, rx_apple_input);
    ...
    compact_apple1.check_input = Some(Box::new(|| {
        compact_apple1.terminal.event_loop();
        if let Ok(mut c) = compact_apple1.terminal.check_input() {
            match c {
                0x0A => c = 0x0D,
                0xBE => c = 0x2E,
                _ => {}
    ...
    *outer.borrow_mut() = Some(Closure::wrap(Box::new(move || {
        ...
        let mut counter = 60;
        compact_apple1.cpu.cycle(false);
        while counter > 0 && !compact_apple1.cpu.completed_operation_cycles() {
            compact_apple1.cpu.cycle(false);
            counter -= 1;
        }
